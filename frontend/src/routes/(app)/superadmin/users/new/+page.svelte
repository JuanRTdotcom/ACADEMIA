<script lang="ts">
  ...
  let roleToAdd = $state('');
  const empresas = $derived(data.empresas as CompanyOption[]);
  const roles = $derived(data.roles as RoleOption[]);
  const modules = $derived(data.modulos_por_empresa?.[form.fid_organizaciones] ?? []);
  ...
  const selectedCompany = $derived(empresas.find((e) => e.id_organizaciones === form.fid_organizaciones));
  const companySlug = $derived(selectedCompany?.slug || 'empresa');
  ...
  $effect(() => {
    const validPermissions = form.fid_permisos.filter((permission) => availablePermissions.has(permission));
    if (validPermissions.length !== form.fid_permisos.length) form.fid_permisos = validPermissions;
  });
...
<section class="flex w-full flex-col gap-6">
  ...
  <form method="POST" action="?/create" use:enhance={create} class="flex flex-col gap-5">
    <Card><div class="mb-5"><h2 class="text-base font-semibold text-ink">{i18n.t('users.assignments')}</h2><p class="mt-1 text-sm text-steel">Selecciona dónde se creará la cuenta y qué capacidades globales tendrá.</p></div><div class="grid gap-4 lg:grid-cols-3"><Select label={i18n.t('users.company')} icon="building-2" bind:value={form.fid_organizaciones} onchange={changeCompany} error={attempted && errors.company ? i18n.t(errors.company) : undefined} disabled={saving} required><option value="">{i18n.t('users.selectCompany')}</option>{#each data.empresas as empresa (empresa.id_organizaciones)}<option value={empresa.id_organizaciones}>{empresa.nombre}</option>{/each}</Select><div class="lg:col-span-2"><Select id="user-create-roles" label={i18n.t('users.roles')} icon="shield-check" bind:value={roleToAdd} onchange={addRole} error={attempted && errors.roles ? i18n.t(errors.roles) : undefined} disabled={saving || !form.fid_organizaciones}><option value="">Selecciona un rol</option>{#each roles.filter((role) => !form.fid_roles.includes(role.id_roles)) as role (role.id_roles)}<option value={role.id_roles}>{role.nombre}</option>{/each}</Select>{#if selectedRoles.length > 0}<div class="mt-3 flex flex-wrap gap-2">{#each selectedRoles as role (role.id_roles)}<button type="button" class="inline-flex items-center gap-1.5 rounded-md border border-primary/35 bg-primary/5 px-2.5 py-1.5 text-sm font-medium text-primary transition hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-50" onclick={() => removeRole(role.id_roles)} disabled={saving}><Icon name={role.icono || 'shield-check'} size={15} />{role.nombre}<Icon name="x" size={14} /></button>{/each}</div>{/if}</div></div></Card>
