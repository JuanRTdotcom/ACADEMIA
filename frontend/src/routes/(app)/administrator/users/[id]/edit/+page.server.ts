...
export const load: PageServerLoad = async (event) => {
  ...
  if (!tienePermiso(usuario.permisos, 'administrator.users.update', 'administrator.users.read')) {
  ...
  try {
    ...
    if (!userResponse.ok) {
      error(userResponse.status, await companyMessage(userResponse, 'users.loadError'));
    }
    if (!optionsResponse.ok) {
      error(optionsResponse.status, await companyMessage(optionsResponse, 'users.loadError'));
    }
...
export const actions: Actions = {
  save: async (event) => {
    ...
    try {
      const response = await companyRequest(event, `/company/current/users/${id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (!response.ok) {
        return fail(response.status, { userMessage: await companyMessage(response, 'users.saveError') });
      }
