...
export const load: PageServerLoad = async (event) => {
  ...
  const response = await companyRequest(
    event,
    `/company/consultation-reasons${query.size ? `?${query}` : ""}`,
  );
  if (!response.ok)
    error(
      response.status,
      await companyMessage(response, "consultationReasons.loadError"),
    );
...
function payload(form: FormData) {
  ...
  return nombre.length >= 2 && nombre.length <= 120 && descripcion.length <= 500
    ? { nombre, descripcion }
    : null;
}
...
export const actions: Actions = {
  create: async (event) => {
    ...
    return data
      ...
      : fail(400, { reasonMessage: "consultationReasons.invalidData" });
  },
  update: async (event) => {
    ...
    return UUID.test(id) && data
      ? mutate(
          event,
          "administrator.consultation_reasons.update",
          `/company/consultation-reasons/${id}`,
          "PATCH",
          data,
        )
      : fail(400, { reasonMessage: "consultationReasons.invalidData" });
  ...
  status: async (event) => {
    ...
    return UUID.test(id) && ["true", "false"].includes(activo)
      ? mutate(
          event,
          "administrator.consultation_reasons.update",
          `/company/consultation-reasons/${id}/status`,
          "PATCH",
          { activo: activo === "true" },
        )
      : fail(400, { reasonMessage: "consultationReasons.invalidData" });
  ...
  delete: async (event) => {
    ...
    return UUID.test(id)
      ? mutate(
          event,
          "administrator.consultation_reasons.delete",
          `/company/consultation-reasons/${id}`,
          "DELETE",
        )
      : fail(400, { reasonMessage: "consultationReasons.invalidData" });
